...
export const SECTIONS: Section[] = [
  {
    id: 'hero',
    page: 'home',
    heading: 'The Turbo web tool that everybody needed… Was just Hamstarted!',
    body: 'A TurboRepo starter with the whole CI/CD chain already wired: lint, typecheck, unit and layout tests and a bundle budget, gating a prerendered deploy to GitHub Pages that refuses to publish if any of it fails. Firebase, Cloudinary, Render and UptimeRobot are plumbed in on their free tiers, so a React and TypeScript app is live in an afternoon. Like TurboHam, it turns up with everything already stuffed in its cheeks and runs on almost nothing.',
    order: 0,
    translations: {
      ja: {
        heading: 'みんなが必要としていたTurbo製Webツール…ハムスタートされました！',
        body: 'CI/CDチェーンがあらかじめ組み込まれた Turborepo スターターキット。\nLint、型チェック、単体テスト・レイアウトテスト、バンドルサイズ制限のどれか1つでも落とせば、GitHub Pages へのプリレンダリングデプロイは自動で停止します。\nFirebase、Cloudinary、Render、UptimeRobot もすべて無料枠で連携済み。React × TypeScript アプリをたった半日で公開できます。\nTurboHam のように、必要なものはすべて「頬袋」に詰め込んで準備万端。ごくわずかなリソースで動き出します！',
      },
    },
  },
  {
    ...
    body: 'A React + TypeScript front end prerendered to static HTML, an Express API with a Firestore-backed admin portal, and a CI pipeline that refuses to deploy if anything fails. Every service it touches has a free tier. Plenty of running, none of it on a wheel.',
    order: 1,
    translations: {
      ja: {
        heading: 'これが何なのか',
        body: '静的HTMLにプリレンダリングされたReact + TypeScriptのフロントエンド、Firestoreを使う管理画面付きのExpress API、そして何か失敗すればデプロイを拒否するCIパイプライン。利用するサービスはすべて無料枠があります。よく走りますが、回し車の上ではありません。',
      },
    },
  },
  {
  ...
  },
  {
    ...
    body: 'A Turborepo with three workspaces. The front end is React and TypeScript on Vite, prerendered to a real HTML file per route so a visitor and a crawler both get content on the first response rather than an empty div. The API is Express with a Firestore-backed admin portal for editing every piece of copy on the site, including the footer. A shared package holds the content model both sides agree on, so a change to it breaks the build rather than the page.',
    order: 1,
    translations: {
      ja: {
        heading: '頬袋の中身',
        body: 'ワークスペース3つの Turborepo です。フロントエンドは Vite 上の React と TypeScript。ルートごとに実際の HTML ファイルへプリレンダリングするので、訪問者にもクローラーにも空の div ではなく中身が最初のレスポンスで届きます。API は Express で、サイト上のすべての文章 — フッターも含めて — を編集できる Firestore 連携の管理画面付き。共有パッケージが両者の合意するコンテンツモデルを持つため、変更するとページではなくビルドが壊れます。',
      },
    },
  },
  {
    ...
    body: "Every push runs the linter, the typechecker, an API suite, a unit suite and a set of layout tests across three viewports, then checks the gzipped bundle against a budget and audits the built output for accessibility and SEO. The prerenderer adds three gates of its own: that every route hydrates without React discarding the markup, that each one carries its own content, and that none of them rewrite that content after load. None of this is slow, either — the browser every check needs is cached across pushes rather than re-downloaded from scratch, so the pipeline's own time goes to the checks themselves. Fail any of it and the deploy simply does not happen. The tests are the cage bars, and they are there for the same reason.",
    order: 2,
    translations: {
      ja: {
        heading: '雑なものを出させないパイプライン',
        body: 'push のたびに Lint、型チェック、API テスト、ユニットテスト、3つのビューポートでのレイアウトテストが走り、gzip 後のバンドルサイズを予算と照合し、ビルド結果のアクセシビリティと SEO を監査します。プリレンダラーはさらに3つの検査を持ちます。各ルートが React にマークアップを捨てられずにハイドレートすること、それぞれが自分のコンテンツを持っていること、読み込み後にそれを書き換えないこと。ここも遅くはありません——各チェックに必要なブラウザは push のたびに再ダウンロードされず、キャッシュされます。パイプラインの時間はチェックそのものに使われます。どれか1つでも落ちればデプロイは実行されません。テストはケージの柵です。理由も同じです。',
      },
    },
  },
  {
